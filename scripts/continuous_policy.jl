function simulate!(s::LciMPC.Simulator{T}; verbose=false) where T
    status = false
...
function exec_policy(p::LciMPC.CIMPC{T,NQ,NU,NW,NC}, traj::LciMPC.Trajectory{T}, t::Int, buffer_time::T) where {T,NQ,NU,NW,NC}
	# reset
	if t == 1
		p.cnt[1] = p.N_sample
		p.q0 .= p.ref_traj.q[1]
